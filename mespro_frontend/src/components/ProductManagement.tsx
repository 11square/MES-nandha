...
import { Plus, Trash2, FolderOpen, Boxes, Tag, Edit, ChevronRight, Search, Package, LayoutGrid, List } from 'lucide-react';
...
export default function ProductManagement({ productCategories = [], onCategoriesChange, sharedProducts = [] }: ProductManagementProps) {
  ...
  const [catView, setCatView] = useState<'grid' | 'table'>('grid');
  const [catFilter, setCatFilter] = useState<'all' | 'with-subs' | 'empty'>('all');
  const [subView, setSubView] = useState<'grid' | 'table'>('table');
    ...
    <div className="px-6 pt-2 pb-4 flex flex-col gap-3 overflow-hidden" style={{ height: 'calc(100dvh - 72px)' }}>
        ...
        <>
          ...
          {selectedSub ? (
            ...
            <>
              ...
              <div className="bg-white rounded-lg border border-gray-200 shadow-sm flex items-center justify-between gap-3 px-3 py-2 flex-shrink-0 flex-wrap">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  <div className="inline-flex items-center gap-0.5 rounded-md border border-slate-200 bg-white p-0.5">
                    {([['all', 'All'], ['with-items', 'With Items'], ['empty', 'Empty']] as const).map(([v, label]) => (
                      <button key={v} type="button" onClick={() => setSubFilter(v as any)} className={`h-7 px-2.5 text-xs font-medium rounded transition-colors ${subFilter === v ? 'bg-blue-600 text-white' : 'text-slate-600 hover:bg-slate-100'}`}>{label}</button>
                    ))}
                  </div>
                  <div className="inline-flex items-center gap-0.5 rounded-md border border-slate-200 bg-white p-0.5">
                    <button type="button" onClick={() => setSubView('grid')} title="Grid view" className={`h-7 w-7 flex items-center justify-center rounded transition-colors ${subView === 'grid' ? 'bg-blue-600 text-white' : 'text-slate-600 hover:bg-slate-100'}`}><LayoutGrid className="w-4 h-4" /></button>
                    <button type="button" onClick={() => setSubView('table')} title="Table view" className={`h-7 w-7 flex items-center justify-center rounded transition-colors ${subView === 'table' ? 'bg-blue-600 text-white' : 'text-slate-600 hover:bg-slate-100'}`}><List className="w-4 h-4" /></button>
                  </div>
                </div>
              </div>
              {(() => {
                ...
                if (filtered.length === 0) {
                  return (<div className="bg-white rounded-lg border border-gray-200 shadow-sm flex-1 min-h-0 flex items-center justify-center"><div className="text-center py-12"><Tag className="w-10 h-10 text-gray-300 mx-auto mb-2" /><p className="text-sm text-gray-500">{subSearch || subFilter !== 'all' ? 'No subcategories match' : 'No subcategories yet'}</p>{!subSearch && subFilter === 'all' && <p className="text-xs text-gray-400 mt-1">Click "Add Subcategory" to get started</p>}</div></div>);
                }
                if (subView === 'grid') {
                  return (
      ...
      <div className="flex-1 min-h-0 flex flex-col gap-3 overflow-hidden">
        <div className="bg-white rounded-lg border border-gray-200 shadow-sm flex items-center justify-between gap-3 px-3 py-2 flex-shrink-0 flex-wrap">
          <div className="relative flex-1 min-w-[200px] max-w-md">
            <Search className="w-4 h-4 text-gray-400 absolute left-2.5 top-1/2 -translate-y-1/2" />
            <Input value={catSearch} onChange={e => setCatSearch(e.target.value)} placeholder="Search categories..." className="h-9 pl-8" />
          </div>
          <div className="flex items-center gap-2 flex-shrink-0">
            <div className="inline-flex items-center gap-0.5 rounded-md border border-slate-200 bg-white p-0.5">
              {([['all', 'All'], ['with-subs', 'With Subs'], ['empty', 'Empty']] as const).map(([v, label]) => (
                <button key={v} type="button" onClick={() => setCatFilter(v as any)} className={`h-7 px-2.5 text-xs font-medium rounded transition-colors ${catFilter === v ? 'bg-blue-600 text-white' : 'text-slate-600 hover:bg-slate-100'}`}>{label}</button>
              ))}
            </div>
            <div className="inline-flex items-center gap-0.5 rounded-md border border-slate-200 bg-white p-0.5">
              <button type="button" onClick={() => setCatView('grid')} title="Grid view" className={`h-7 w-7 flex items-center justify-center rounded transition-colors ${catView === 'grid' ? 'bg-blue-600 text-white' : 'text-slate-600 hover:bg-slate-100'}`}><LayoutGrid className="w-4 h-4" /></button>
              <button type="button" onClick={() => setCatView('table')} title="Table view" className={`h-7 w-7 flex items-center justify-center rounded transition-colors ${catView === 'table' ? 'bg-blue-600 text-white' : 'text-slate-600 hover:bg-slate-100'}`}><List className="w-4 h-4" /></button>
            </div>
          </div>
        </div>
        ...
        {(() => {
          ...
          const filtered = categories.filter(c => {
            ...
            if (catFilter === 'with-subs' && c.subcategories.length === 0) return false;
            if (catFilter === 'empty' && c.subcategories.length > 0) return false;
            return true;
          });
          if (categories.length === 0) {
            return (<Card className="shadow-sm"><CardContent className="py-16 text-center"><FolderOpen className="w-12 h-12 text-gray-300 mx-auto mb-3" /><p className="text-gray-500 font-medium">No categories yet</p><p className="text-xs text-gray-400 mt-1">Click "Add Category" to get started</p></CardContent></Card>);
          }
          if (filtered.length === 0) {
            return (<Card className="shadow-sm"><CardContent className="py-16 text-center"><Search className="w-10 h-10 text-gray-300 mx-auto mb-2" /><p className="text-sm text-gray-500">No categories match your search</p></CardContent></Card>);
          }
          if (catView === 'table') {
            return (
              <div className="bg-white rounded-lg border border-gray-200 shadow-sm flex-1 min-h-0 flex flex-col overflow-hidden">
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50 border-b border-gray-200 sticky top-0 z-10">
                      <tr className="text-left text-xs font-semibold text-gray-600 uppercase tracking-wide">
                        ...
                        <th className="px-4 py-2.5 text-right">Subcategories</th>
                        <th className="px-4 py-2.5 text-right">Items</th>
                        <th className="px-4 py-2.5 text-right w-32">Actions</th>
                      </tr>
                    </thead>
                      ...
                      {filtered.map((cat, i) => {
                        ...
                        return (
                          <tr key={cat.id} onClick={() => { setSelectedCatId(cat.id); setSelectedSub(null); setSubSearch(''); }} className="border-b border-gray-100 hover:bg-blue-50/50 cursor-pointer group">
                            <td className="px-4 py-2.5 text-gray-500">{i + 1}</td>
                            <td className="px-4 py-2.5"><div className="flex items-center gap-2"><div className="w-7 h-7 bg-blue-100 rounded-lg flex items-center justify-center flex-shrink-0"><FolderOpen className="w-3.5 h-3.5 text-blue-600" /></div><span className="font-medium text-gray-900">{cat.name}</span></div></td>
                            <td className="px-4 py-2.5 text-right"><span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-violet-100 text-violet-700 text-xs font-medium"><Tag className="w-3 h-3" />{cat.subcategories.length}</span></td>
